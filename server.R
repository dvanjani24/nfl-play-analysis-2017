...
remove_these <- 
  union(x = which(is.na(nfl_data_x$EPA) == TRUE),
        y = which(is.na(nfl_data_x$WPA) == TRUE))

graph1_data <- nfl_data_x[-(remove_these), ]
...
function(input, output) {
  ...
  output$WPA_YAC_plot <- renderPlot({
    nfl_data = filter(nfl_data, PassOutcome == "Complete")
    ggplot(subset(nfl_data, posteam == input$which_variable), 
  ...
  output$passlocation_WPA_plot <- renderPlotly({
    ...
    p <- ggplot(subset(newdat, posteam == input$which_variable), 
